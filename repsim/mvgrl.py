...
class GCN(nn.Module):
    ...
    def forward(self, data):
        ...
        feat, _ = torch_geometric.utils.to_dense_batch(
...
def local_global_loss_(l_enc, g_enc, batch, measure, mask):
    ...
    for idx, m in enumerate(mask):
        msk[idx * max_nodes + m : idx * max_nodes + max_nodes, idx] = 0.0
...
def global_global_loss_(g1_enc, g2_enc, measure):
    ...
    for graphidx in range(num_graphs):
        pos_mask[graphidx][graphidx] = 1.0
        neg_mask[graphidx][graphidx] = 0.0
...
def trainsparse(dataset, gpu, num_layer=4, epoch=40, batch=64):
    nb_epochs = epoch
    batch_size = batch
    patience = 20
    lr = 0.001
    ...
    for epoch in range(nb_epochs):
        ...
        for data in loader:
            ...
            batch = torch.LongTensor(
                np.repeat(np.arange(batch.shape[0]), max_nodes)
            ).cuda()
            ...
            for b in batch:
                for e in epoch:
                    for i in range(5):
                        ...
                        np.random.seed(seed)
                        print(
                            f"Dataset: {d}, Layer:{l}, Batch: {b}, Epoch: {e}, Seed: {seed}"
                        )
                        train(d, gpu, l, e, b)
        print("################################################")
