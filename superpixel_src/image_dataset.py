...
class ImageDataset(InMemoryDataset):
    def __init__(
        ...
        self.traindata = None
        self.testdata = None
        super(ImageDataset, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def processed_file_names(self):
        return [
            "%s_training.pt" % self.processed_file_prefix,
    ...
    def download(self):
        ...
        if self.name == "CIFAR10":
            data_train = datasets.CIFAR10(
                root=self.raw_dir, transform=transform, train=True, download=True
            )
            data_test = datasets.CIFAR10(
                root=self.raw_dir, transform=transform, train=False, download=True
            )
        ...
        def transform_data(data_loader, edge_index, edge_attr):
            ...
            for image, label in iter(data_loader):
                x = (
                    image[0]
                    .permute([1, 2, 0])
                    .view(num_row * num_col, image[0].size()[0])
                )
                if self.coord:
