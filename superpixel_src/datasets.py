...
def get_dataset(
    name, sparse=True, feat_str="deg+ak3+reall", root=None, aug=None, aug_ratio=None
):
    ...
    if "MNIST" in name or "CIFAR" in name:
        if name == "MNIST_SUPERPIXEL":
            train_dataset = MNISTSuperpixels(
                path, True, pre_transform=pre_transform, transform=T.Cartesian()
            )
            test_dataset = MNISTSuperpixels(
                path, False, pre_transform=pre_transform, transform=T.Cartesian()
            )
        else:
