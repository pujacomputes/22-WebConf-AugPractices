...
import tqdm
from models import MNIST_GNN, SimSiam, SimCLR
from datasets_mnist import *
from knn_monitor import knn_monitor
import pdb
import sys

def mixup(input, alpha=0.9):
    unif = torch.distributions.uniform.Uniform(alpha, 1.0)
    roll_idx_1 = torch.roll(torch.arange(start=0, end=input.shape[0]),shifts=1)
    roll_idx_2 = torch.roll(torch.arange(start=0, end=input.shape[0]),shifts=2)
    lam_1 = unif.sample([input.shape[0]]).to(device=input.device)
    lam_2 = unif.sample([input.shape[0]]).to(device=input.device)
    lam_expanded_1 = lam_1.view([-1] + [1]*(input.dim()-1))
    lam_expanded_2 = lam_2.view([-1] + [1]*(input.dim()-1))
    
    #perform mix-up according to the rolled_idx and uniform sampling
    output_1 = lam_expanded_1 * input + (1. - lam_expanded_1) * input[roll_idx_1]
    output_2 = lam_expanded_2 * input + (1. - lam_expanded_2) * input[roll_idx_2]
...
def run_unsupervised(
    ...
    try:
        with tqdm.tqdm(
            ...
            for i in range(args.epochs):
                ...
                for data in train_loader:
                    ...
                    embeds = model.encoder(view_1.x, view_1.edge_index, view_1.batch)
                    e_1, e_2 = mixup(embeds)

                    z1 = model.predictor(e_1)
                    z2 = model.predictor(e_2)
                    
                    optimizer.zero_grad()
                    # compute embeddings
                    L = model.NT_XentLoss(z1, z2)
                    # calculate loss
                    L.backward()
                    # take optimizer step
                    if args.clip_grad:
                ...
                torch.save(
                    ckpt,
                    "CKPTS_SIMCLR_DACL/{model_id}_{aug_type}_{aug_ratio}_epoch_{e}.pth".format(
                        model_id=model_id,
                ...
                if knn_acc > best_epoch:
                    ...
                    torch.save(
                        best_ckpt,
                        "CKPTS_SIMCLR_DACL/best_{model_id}_{aug_type}_{aug_ratio}.pth".format(
                            model_id=model_id,
                            aug_type=args.aug_type,
                            aug_ratio=args.aug_ratio,
                        ),
        ...
        torch.save(
            final_ckpt,
            "CKPTS_SIMCLR_DACL/resume_{model_id}_{aug_type}_{aug_ratio}.pth".format(
    ...
    torch.save(
        final_ckpt,
        "CKPTS_SIMCLR_DACL/final_{model_id}_{aug_type}_{aug_ratio}.pth".format(
            model_id=model_id, aug_type=args.aug_type, aug_ratio=args.aug_ratio
        ),
    )
...
def main():
    ...
    model = SimCLR(backbone=backbone, args=args).to(device)
