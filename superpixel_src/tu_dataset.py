...
class TUDatasetExt(TUDataset):
    ...
    def __init__(
        ...
        super(TUDatasetExt, self).__init__(
            root, name, transform, pre_transform, pre_filter, use_node_attr
        )
...
def drop_nodes(data, aug_ratio=0.2):
...
def permute_edges(data, aug_ratio):
    ...
    edge_index = np.concatenate(
        (
            edge_index[
                :, np.random.choice(edge_num, (edge_num - permute_num), replace=False)
            ],
            idx_add,
        ),
        axis=1,
