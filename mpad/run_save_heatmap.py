...
def main():
    ...
    sim = SimSiam(
        backbone=model,
        project_dim=args.hidden,
        bottle_neck_dim=args.bottleneck,
    ).to(device)

    ckpt = torch.load(
        "/home/sc/eslubana/graphssl/mpad/mpad/NLP_CKPTS/subjectivity_gcn_ADAM_64_30_128_-_-_-_-_window_2_rep_0/epoch_24.pth",
        map_location="cpu",
    )
    ...
    for i in range(n_val_batches):
        with torch.no_grad():
            ...
            a = adj_val[i].to(device).to_dense().unsqueeze(0)
            f = features_val[i].to(device)
            b = batch_n_graphs_val[i].to(device)
    ...
    for i in range(n_test_batches):
        with torch.no_grad():
            a = adj_test[i].to(device).to_dense().unsqueeze(0)
            f = features_test[i].to(device)
            b = batch_n_graphs_test[i].to(device)
