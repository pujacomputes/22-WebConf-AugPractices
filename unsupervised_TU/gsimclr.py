...
if __name__ == "__main__":
    ...
    accuracies = {"val": [], "test": [],"val_std":[], "test_std":[]}
    epochs = 20
    log_interval = 10
    batch_size = 128
    # batch_size = 512
    ...
    model.eval()
    emb, y = model.encoder.get_embeddings(dataloader_eval)
    val_acc,val_acc_std, acc, acc_std = evaluate_embedding(emb, y)
    accuracies["val"].append(val_acc)
    accuracies["val_std"].append(val_acc_std)
    accuracies["test"].append(acc)
    accuracies["test_std"].append(acc_std)
    try:
    ...
    with open("logs/log_" + args.DS + "_" + args.aug, "a+") as f:
        ...
        f.write("\n")
    log_str = "{dataset}_{layers}_{dim}_{val_acc:.4f}_{val_std:.4f}_{acc:.4f}_{acc_std:.4f}".format(dataset=args.DS, layers=args.num_gc_layers, dim=args.hidden_dim, val_acc=val_acc, val_std=val_acc_std, acc=acc,acc_std=acc_std)

    with open("inductive_bias.csv", "a+") as f:
        f.write("{}\n".format(log_str))
