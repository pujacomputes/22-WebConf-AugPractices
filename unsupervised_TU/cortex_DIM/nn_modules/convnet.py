...
class Convnet(nn.Module):
    ...
    def create_conv_layers(self, shape, conv_args):
        ...
        for i, (dim_out, f, s, p, batch_norm, dropout, nonlinearity, pool) in enumerate(
            conv_args
        ):
            ...
            if pool:
                ...
                conv_block.add_module(
                    name + "pool", Pool(kernel_size=kernel, stride=stride)
                )
...
class FoldedConvnet(Convnet):
    ...
    def create_layers(self, shape, crop_size=8, conv_args=None, fc_args=None):
        ...
        if dim_x != dim_y:
            raise ValueError(
                "x and y dimensions must be the same to use Folded encoders."
            )
