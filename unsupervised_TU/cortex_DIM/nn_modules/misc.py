...
class Unfold(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = (
            self.unfold(x)
            .reshape(N, -1, self.fold_size, self.fold_size, self.n_locs * self.n_locs)
            .permute(0, 4, 1, 2, 3)
...
class Fold(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = (
            x.reshape(-1, self.n_locs * self.n_locs, dim_c, dim_x * dim_y)
            .permute(0, 2, 3, 1)
            .reshape(-1, dim_c * dim_x * dim_y, self.n_locs, self.n_locs)
            .contiguous()
        )
        return x
