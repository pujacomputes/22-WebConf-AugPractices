...
def logistic_classify(x, y):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        train_embs, train_lbls = (
            torch.from_numpy(train_embs).cuda(),
            torch.from_numpy(train_lbls).cuda(),
        )
        test_embs, test_lbls = (
            torch.from_numpy(test_embs).cuda(),
            torch.from_numpy(test_lbls).cuda(),
        )
...
def randomforest_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            ...
            classifier = GridSearchCV(
                RandomForestClassifier(), params, cv=5, scoring="accuracy", verbose=0
            )
        else:
...
def linearsvc_classify(x, y, search):
    ...
    return np.mean(accuracies_val),np.std(accuracies_val), np.mean(accuracies), np.std(accuracies)
...
def evaluate_embedding(embeddings, labels, search=True):
    ...
    val_acc, val_acc_std, acc, acc_std  = linearsvc_classify(x, y, search)
    ...
    return val_acc, val_acc_std, acc, acc_std
