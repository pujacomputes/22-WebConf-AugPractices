...
class TUDatasetExt(TUDataset):
    ...
    def get(self, idx):
        ...
        if self.aug == 'dropN':
            ...
            data = mixup(data, self.aug_ratio)
        else:
...
def mixup(data, alpha):
    ...
    """
    TODO: Converting to adj and then to adjacency matrix is likely expensive!
    Find a pyg method of mixing up edge indices later!!

    """
    adj = geom.utils.to_dense_adj(data.edge_index, data.batch)
    x,mask = geom.utils.to_dense_batch(data.x, data.batch)

    #mix adjacency 
    beta = torch.distributions.beta.Beta(alpha, alpha)
    randind = torch.randperm(adj.shape[0], device=adj.device)
    lam = beta.sample([adj.shape[0]]).to(device=adj.device)
    lam = torch.max(lam, 1. - lam)
    lam_expanded = lam.view([-1] + [1]*(adj.dim()-1))
    mixed_adj = lam_expanded * adj + (1. - lam_expanded) * adj[randind]

    #convert back to edge_index and edge_attr    
    data.edge_index = mixed_adj.nonzero(as_tuple=False).t()
    data.edge_attr = mixed_adj[edge_index[0], edge_index[1]] 
    
    #mix features to match!
    beta = torch.distributions.beta.Beta(alpha, alpha)
    randind = torch.randperm(x.shape[0], device=x.device)
    lam = beta.sample([x.shape[0]]).to(device=x.device)
    lam = torch.max(lam, 1. - lam)
    lam_expanded = lam.view([-1] + [1]*(x.dim()-1))
    mixed_x = lam_expanded * x + (1. - lam_expanded) * x[randind]
    
    #adjust the mask to match the mixed samples
    mixed_mask = torch.logical_or(input['mask'],input['mask'][randind])
    data.mask = torch.cat([torch.Tensor([e_num] * m.sum()).long() for e_num,m in enumerate(mixed_mask)])
    data.randind = randind
    data.lam = lam
    return data
